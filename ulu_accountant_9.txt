...
with tab11:
    ...
    if tdf.empty or len(tdf) < 2:
        ...
        radr1, radr2 = st.columns(2)
        radr1.metric(f"Recent 3-Month ADR ({', '.join(recent3['label'])})", f"RM {recent3_adr:,.0f}",
                     delta=f"{recent3_adr - alltime_adr:+,.0f} vs all-time average",
                     help="Weighted by actual nights sold, not a simple average of monthly figures. "
                          "Shows current pricing performance separate from older, pre-revision bookings "
                          "still sitting in the historical blend.")
        radr2.metric("All-Time ADR", f"RM {alltime_adr:,.0f}",
                     help="Blended across your full booking history, including any bookings made before "
                          "recent rate revisions or long-stay discounted stays.")
