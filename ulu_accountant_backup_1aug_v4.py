...
# Uploaded to Supabase Storage (survives Streamlit Cloud restarts/redeploys)
# AND saved locally to an "Accounting Folder" for the accountant/auditor,
# same dual-persistence pattern already used for CapEx receipts.
# Create the bucket once in Supabase: Storage → New bucket → "ulu1-attachments" (private).
# ─────────────────────────────────────────────
PAYMENT_BUCKET = "ulu1-attachments"
ACCOUNTING_FOLDER_NAME = "Accounting Folder - Payment Vouchers"
...
def upload_payment_file(file_bytes, file_name, kind):
    ...
    kind: 'claim_invoice' or 'proof' — used to namespace the storage path.
...
def save_payment_attachment_local(file_bytes, file_name, subfolder, voucher_no):
    """Save a copy into the local Accounting Folder for direct auditor/accountant
    access via OneDrive sync. Only persists when running locally — Streamlit
    Cloud's disk resets on redeploy, which is why Supabase Storage (above) is
    the source of truth and this is a convenience copy."""
    if not file_bytes or _is_running_on_cloud():
        return None
    folder = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           ACCOUNTING_FOLDER_NAME, subfolder)
    os.makedirs(folder, exist_ok=True)
    ext = Path(file_name).suffix or ".bin"
    fn = f"{voucher_no}_{subfolder.replace(' ','_')}{ext}"
...
# Moved here (was previously defined at the bottom of the file, near Tab 10) —
# it must be defined before the "Airbnb CSV & Reconcile" sub-tab under Scan
# Receipts runs, since Streamlit executes the script top-to-bottom on every
# rerun. Defining it after the call site caused "name 'parse_airbnb_csv' is
# not defined" whenever that sub-tab loaded before Tab 10 did.
# ─────────────────────────────────────────────
DIRECT_INCOME_TYPES = [
...
with tab9:
    ...
    with pv_subtab1:
        ...
        pv_invoice_file = fcol1.file_uploader("📎 Attach Claim Invoice", type=["jpg","jpeg","png","pdf"], key="pv_invoice_upload")
        pv_proof_file   = fcol2.file_uploader("📎 Attach Proof of Payment", type=["jpg","jpeg","png","pdf"], key="pv_proof_upload")
        ...
        if st.button("💾 Save Payment & Generate Voucher", type="primary", key="btn_save_pv"):
            if not pv_payee or pv_amount <= 0:
                ...
                invoice_bytes = pv_invoice_file.read() if pv_invoice_file else None
                proof_bytes   = pv_proof_file.read() if pv_proof_file else None

                invoice_storage_path = upload_payment_file(invoice_bytes, pv_invoice_file.name, "claim_invoice") if invoice_bytes else None
                proof_storage_path   = upload_payment_file(proof_bytes, pv_proof_file.name, "proof") if proof_bytes else None

                # Convenience local copy for the accountant/auditor (only persists when run locally)
                if invoice_bytes:
                    save_payment_attachment_local(invoice_bytes, pv_invoice_file.name, "Claim Invoices", voucher_no)
                if proof_bytes:
                    save_payment_attachment_local(proof_bytes, pv_proof_file.name, "Proof of Payment", voucher_no)
                ...
                if invoice_storage_path or proof_storage_path:
                    st.caption("Attachment(s) saved to secure storage" +
                               (" and the local Accounting Folder." if not _is_running_on_cloud() else "."))
        ...
        if not pv_rows:
            ...
            for r in [dict(r) for r in pv_rows]:
                ...
                with st.expander(header_label, expanded=False):
                    ...
                    if r.get("claim_invoice_path") or r.get("proof_of_payment_path"):
                        att1, att2 = st.columns(2)
                        if r.get("claim_invoice_path"):
                            inv_url = get_payment_file_url(r["claim_invoice_path"])
                            if inv_url:
                                att1.markdown(f"📎 [Claim Invoice]({inv_url})")
                        else:
                            att1.caption("No claim invoice attached")
                        if r.get("proof_of_payment_path"):
                            proof_url = get_payment_file_url(r["proof_of_payment_path"])
                            if proof_url:
                                att2.markdown(f"📎 [Proof of Payment]({proof_url})")
                        else:
