        ...
        if not comp_rows:
            ...
            if primary:
                ...
                if not primary_rates.empty:
                    ...
                    pc1.metric(f"ULU 1's Latest 12-Month ADR", f"RM {recent12_adr:,.0f}")
                    pc2.metric(f"⭐ {primary['name']} (Primary Benchmark)", f"RM {primary_avg:,.0f}")
                    gap_pct = ((recent12_adr - primary_avg) / primary_avg * 100) if primary_avg else 0
            ...
            latest_comp_avg = benchmark_df.sort_values("date_checked", ascending=False).head(8)["rate_rm"].astype(float).mean()

            mc1, mc2, mc3 = st.columns(3)
            mc1.metric("ULU 1's Latest 12-Month ADR", f"RM {recent12_adr:,.0f}",
                       help="Trailing 12 months — matches the 6-month competitor refresh cadence better "
                            "than a single-month or all-time legacy figure.")
            mc2.metric("Same-Typology Market Avg (last 8 checks, outliers excluded)", f"RM {latest_comp_avg:,.0f}")
